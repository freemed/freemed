...
DROP PROCEDURE IF EXISTS preport_PatientAccountActivity_en_US ;
DELIMITER //

# Function: preport_PatientAccountActivity_en_US
#
#	Patient account activity for a specified date range.
#
CREATE PROCEDURE preport_PatientAccountActivity_en_US ( IN patientId INT UNSIGNED, IN beginDate DATE, IN endDate DATE )
BEGIN
	SET @sql = CONCAT(
		"SELECT ",
...
DELETE FROM `patient_reporting` WHERE report_sp = 'preport_PatientAccountActivity_en_US';

INSERT INTO `patient_reporting` (
		report_name,
		report_uuid,
		report_locale,
		report_desc,
		...
		'7ab2f3da-25bd-4617-952a-bf411544cc69',
		'en_US',
		'Patient account activity for a specified date range.',
		'preport_PatientAccountActivity_en_US',
		2,
