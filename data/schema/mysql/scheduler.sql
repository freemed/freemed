...
CREATE TRIGGER scheduler_Delete
	...
	FOR EACH ROW BEGIN
		...
		END IF;
		INSERT INTO systemnotification ( stamp, nuser, ntext, nmodule, npatient, naction ) VALUES ( NOW(), 0, CONCAT( LPAD( OLD.calhour, 2, '0' ), ':', LPAD( OLD.calminute, 2, '0' ), ' (', OLD.calduration, 'm) - ', OLD.calprenote ), 'scheduler', OLD.calpatient, 'DELETE' );
	END;
//
...
CREATE TRIGGER scheduler_Insert
	...
	FOR EACH ROW BEGIN
		...
		END IF;
		INSERT INTO systemnotification ( stamp, nuser, ntext, nmodule, npatient, naction ) VALUES ( NEW.calcreated, NEW.user, CONCAT( LPAD( NEW.calhour, 2, '0' ), ':', LPAD( NEW.calminute, 2, '0' ), ' (', NEW.calduration, 'm) - ', NEW.calprenote ), 'scheduler', NEW.calpatient, 'NEW' );
	END;
//
