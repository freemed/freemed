...
// Ver:		$Id: jpgraph_gantt.php,v 1.27 2002/03/31 22:23:23 aditus Exp $
//
...
// PUBLIC METHODS	
	...
	for ( $i = 0, $ofs = 1 - strftime('%w'); $i < 7; $i++, $ofs++ ){
	...
	for ( $i = 0, $ofs = 1 - strftime('%m'); $i < 12; $i++, $ofs++ ){
	    list($short ,$full) = explode('|', strftime('%b|%B', strtotime("$ofs month")));
...
// PRIVATE Methods
	...
	if( $aDate < $this->iStartDate || $aDate > $this->iEndDate )
	    JpGraphError::Raise("Date is outside specified scale range.");
