...
class PatientReporting extends SupportModule {

	var $MODULE_NAME = "Patient Reporting";
	var $MODULE_VERSION = "0.1";
	var $MODULE_FILE = __FILE__;
	var $MODULE_UID = "42781bcd-8f69-4752-8be0-a4360a99612e";
	var $MODULE_HIDDEN = true;

	var $PACKAGE_MINIMUM_VERSION = '0.8.0';

	var $table_name = "patient_reporting";
	...
	public function GetReports ( $locale = NULL ) {
		$query = "SELECT report_name, report_desc, report_uuid FROM patient_reporting WHERE report_locale=". $GLOBALS['sql']->quote( $locale == NULL ? DEFAULT_LANGUAGE : $locale ). " ORDER BY report_name";
	...
	public function GetReportParameters ( $uuid, $flatten = true ) {
		$query = "SELECT * FROM patient_reporting WHERE report_uuid=".$GLOBALS['sql']->quote( $uuid );
	...
	public function GenerateReport ( $uuid, $format, $param ) {
		...
		if (!$report['report_name']) { return false; }
                ...
                // Import patient param
                $pass[] = $GLOBALS['sql']->quote( $param[0] + 0 );

		$s = CreateObject('org.freemedsoftware.api.Scheduler');
		foreach ($report['params'] AS $k => $v) {
			if ( !$v['optional'] and !$param[$k + 1] ) {
			...
			switch ($v['type']) {
				...
				default:
				$pass[] = $GLOBALS['sql']->quote( $param[$k + 1] );
				break;
			}
		}
...
} // end class PatientReporting

register_module("PatientReporting");
