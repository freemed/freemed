...
switch ($action) {
 case "":
  ...
  if (date_in_the_past($selected_date))
    ...
    $display_buffer .=
	( !$travel ? freemed_patient_box ($this_patient) :
	"
	<DIV COLOR=\"#000000\" ALIGN=\"CENTER\">
		<FONT COLOR=\"#ff0000\">
		<B>Travel</B>
		</FONT>	
	</DIV>" )."
    ...
    ";
    fc_generate_calendar_mini($selected_date,
     "$page_name?patient=$patient&room=$room&type=$type&travel=$travel");
    $display_buffer .= " 
 ...
 case "step2":
   ...
   $display_buffer .= "
     ...
     <INPUT TYPE=HIDDEN NAME=\"travel\"   VALUE=\"".prepare($travel)."\">
     <INPUT TYPE=HIDDEN NAME=\"selected_date\"
      VALUE=\"".prepare($selected_date)."\">
