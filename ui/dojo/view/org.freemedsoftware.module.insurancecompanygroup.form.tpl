	...
	var m = {
		...
		initialLoad: function ( ) {
			...
			dojo.io.bind({
				...
				url: "<!--{$relay}-->/org.freemedsoftware.module.insurancecompanygroup.GetRecord",
		...
		submit: function ( ) {
			...
			if (m.validate( myContent )) {
				dojo.io.bind({
					...
					url: "<!--{$relay}-->/org.freemedsoftware.module.insurancecompanygroup.<!--{if $id}-->mod<!--{else}-->add<!--{/if}-->",
					load: function ( type, data, evt ) {
...
<h3><!--{t}-->Insurance Company Group<!--{/t}--></h3>
