...
<script type="text/javascript">
	var m = {
		...
		validate: function ( content ) {
			var r = true;
			var m = "";
			if ( content.clmtpname.length < 2 ) {
				m += "<!--{t}-->You must enter a name.<!--{/t}-->\n";
				r = false;
			}
			if ( content.clmtpdescrip.length < 2 ) {
		...
		initialLoad: function ( ) {
			...
			dojo.io.bind({
				...
				url: "<!--{$relay}-->/org.freemedsoftware.module.claimtypes.GetRecord",
		...
		submit: function ( ) {
			...
			if (m.validate( myContent )) {
				dojo.io.bind({
					...
					url: "<!--{$relay}-->/org.freemedsoftware.module.claimtypes.<!--{if $id}-->mod<!--{else}-->add<!--{/if}-->",
					load: function ( type, data, evt ) {
