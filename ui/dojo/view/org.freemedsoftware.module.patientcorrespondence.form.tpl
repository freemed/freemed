	...
	var letters = {
		...
		initialLoad: function ( ) {
			...
			dojo.io.bind({
				...
				url: "<!--{$relay}-->/org.freemedsoftware.module.PatientCorrespondence.GetRecord",
		...
		submit: function ( ) {
			...
			if (letters.validate( myContent )) {
				dojo.io.bind({
					...
					url: "<!--{$relay}-->/org.freemedsoftware.module.PatientCorrespondence.<!--{if $id}-->mod<!--{else}-->add<!--{/if}-->",
					load: function ( type, data, evt ) {
...
<h3><!--{t}-->Patient Correspondence<!--{/t}--></h3>
