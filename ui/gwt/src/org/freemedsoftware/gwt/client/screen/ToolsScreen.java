...
public class ToolsScreen extends ScreenInterface {
	...
	public static ToolsScreen getInstance() {
		...
		if (toolsScreenList.size() < AppConstants.MAX_TOOLS_TABS) {
			...
			toolsScreen = toolsScreenList
					.get(AppConstants.MAX_TOOLS_TABS - 1);
		}
		return toolsScreen;
	}
	...
	public ToolsScreen() {
		...
		toolTable.setTableRowClickHandler(new TableRowClickHandler() {
			...
			public void handleRowClick(HashMap<String, String> data, int col) {
				String uuid = data.get("tool_uuid");
				thisToolUUID = uuid;
				getToolInformation(uuid);
			}
		});
		...
		// Tool label
		paramContainer.add(thisToolName);

		toolParametersTable = new FlexTable();
		paramContainer.add(toolParametersTable);
		toolParametersTable.setVisible(false);
		toolParametersTable.setSize("100%", "100%");

		toolActionPanel = new HorizontalPanel();
		toolActionPanel.setVisible(false);
	...
	protected void runTool() {
		...
		Window.open(Util.getJsonRequest(
				"org.freemedsoftware.module.Tools.GenerateTool",
				new String[] { thisToolUUID, type,
						JsonUtil.jsonify(getParameters()) }), "Tool", "");
	}
	...
	protected void populateToolParameters(HashMap<String, String> data) {
		...
		thisToolName.setText(data.get("tool_name"));
		...
		toolActionPanel.setVisible(true);
	}
